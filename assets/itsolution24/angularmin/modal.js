...
window.angularApp.factory("TallaCreateModal", ["API_URL", "window", "jQuery", "$http", "$uibModal", "$sce", "$rootScope", function (API_URL, window, $, $http, $uibModal, $sce, $scope) {
    return function($scope) {
        var uibTallaModalInstance = $uibModal.open({
            animation: true,
            ariaLabelledBy: "modal-title",
            ariaDescribedBy: "modal-body",
            template: "<div class=\"modal-header\">" +
                            "<button ng-click=\"closeTallaCreateModal();\" type=\"button\" class=\"close talla-create-modal\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button>" +
                           "<h3 class=\"modal-title\" id=\"modal-title\"><span class=\"fa fa-fw fa-plus\"></span> {{ modal_title }}</h3>" +
                        "</div>" +
                        "<div class=\"modal-body\" id=\"modal-body\">" +
                            "<div bind-html-compile=\"rawHtml\">Loading...</div>" +
            ...
            controller: function ($scope, $uibModalInstance) {
                $http({
                  url: window.baseUrl + "/_inc/talla.php?action_type=CREATE",
                  method: "GET"
                })
                .then(function(response, status, headers, config) {
                    $scope.modal_title = "Create New Talla";
                    $scope.rawHtml = $sce.trustAsHtml(response.data);
                ...
                // Submit Box Form
                $(document).delegate("#create-talla-submit", "click", function(e) {
                    e.stopImmediatePropagation();
                    e.stopPropagation();
                    e.preventDefault();
                    ...
                    then(function(response) {
                        ...
                        .then(function(value) {
                            ...
                            // Cerrar ventana modal
                            $scope.closeTallaCreateModal();
                            $(document).find(".close").trigger("click");

                            // insert talla into select2
                            var select = $(document).find('#talla_id');
                            if (select.length) {
                                var option = $('<option></option>').
                                     attr('selected', true).
                                     text(response.data.talla.talla_name).
                                     val(response.data.id);
                                option.appendTo(select);
                                select.trigger('change');
                            }

                            // increase talla count
                            var tallaCount = $(document).find("#talla-count h3");
                            if (tallaCount) {
                                tallaCount.text(parseInt(tallaCount.text()) + 1);
                            }

                            // Devolver la llamada
                            if ($scope.TallaCreateModalCallback) {
                                $scope.tallaCreateModalCallback($scope);
                            }
                        });
                ...
                $scope.closeTallaCreateModal = function () {
                    uibTallaModalInstance.dismiss("cancel");
                };
            },
            scope: $scope,
            size: "md",
        ...
        uibTallaModalInstance.result.catch(function () { 
            uibTallaModalInstance.close(); 
        });
    };
}]);
window.angularApp.factory("TallaDeleteModal", ["API_URL", "window", "jQuery", "$http", "$uibModal", "$sce", "$rootScope", function (API_URL, window, $, $http, $uibModal, $sce, $scope) {
    return function(talla) {
        var uibModalInstance = $uibModal.open({
            ...
            ariaDescribedBy: "modal-body",
            template: "<div class=\"modal-header\">" +
                            "<button ng-click=\"closeTallaDeleteModal();\" type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button>" +
                           "<h3 class=\"modal-title\" id=\"modal-title\"><span class=\"fa fa-fw fa-trash\"></span> {{ modal_title }}</h3>" +
                        "</div>" +
                        "<div class=\"modal-body\" id=\"modal-body\">" +
                            "<div bind-html-compile=\"rawHtml\">Loading...</div>" +
            ...
            controller: function ($scope, $uibModalInstance) {
                $http({
                  url: window.baseUrl + "/_inc/talla.php?talla_id=" + talla.talla_id + "&action_type=DELETE",
                  method: "GET"
                })
                .then(function(response, status, headers, config) {
                    $scope.modal_title = talla.talla_name;
                    $scope.rawHtml = $sce.trustAsHtml(response.data);
                    ...
                    window.swal("Oops!", response.data.errorMsg, "error")
                    .then(function() {
                        $scope.closeTallaDeleteModal();
                    });
                });

                $(document).delegate("#talla-delete", "click", function(e) {
                ...
                $scope.closeTallaDeleteModal = function () {
                    $uibModalInstance.dismiss("cancel");
                };
            },
            scope: $scope,
...
window.angularApp.factory("TallaEditModal", ["API_URL", "window", "jQuery", "$http", "$uibModal", "$sce", "$rootScope", function (API_URL, window, $, $http, $uibModal, $sce, $scope) {
    return function(talla) {
        var supId;
        var uibModalInstance = $uibModal.open({
            animation: true,
            ...
            ariaDescribedBy: "modal-body",
            template: "<div class=\"modal-header\">" +
                            "<button ng-click=\"closeTallaEditModal();\" type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button>" +
                           "<h3 class=\"modal-title\" id=\"modal-title\"><span class=\"fa fa-fw fa-pencil\"></span> {{ modal_title }}</h3>" +
                        "</div>" +
                        "<div class=\"modal-body\" id=\"modal-body\">" +
                            "<div bind-html-compile=\"rawHtml\">Loading...</div>" +
            ...
            controller: function ($scope, $uibModalInstance) {
                $http({
                  url: window.baseUrl + "/_inc/talla.php?talla_id=" + talla.talla_id + "&action_type=EDIT",
                  method: "GET"
                })
                .then(function(response, status, headers, config) {
                    $scope.modal_title = talla.talla_name;
                ...
                $(document).delegate("#talla-update", "click", function(e) {
                ...
                $scope.closeTallaEditModal = function () {
                    $uibModalInstance.dismiss("cancel");
                };
            },
            scope: $scope,
