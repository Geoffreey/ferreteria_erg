window.angularApp.controller("TallaController", [
  "$scope",
  "API_URL",
  "window",
  "jQuery",
  ...
  function (
      ...
      columnDefs: [
        ...
        { className: "text-center", targets: [0, 1, 2, 3, 4, 5, 6] },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#talla-talla-list thead tr th:eq(0)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#talla-talla-list thead tr th:eq(1)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#talla-talla-list thead tr th:eq(2)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#talla-talla-list thead tr th:eq(3)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#talla-talla-list thead tr th:eq(4)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#talla-talla-list thead tr th:eq(5)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#talla-talla-list thead tr th:eq(6)").html()
            );
          },
        },
      ],
      ...
      buttons: [
        {
          ...
          customize: function (win) {
            $(win.document.body)
              .css("font-size", "10pt")
              .append("<div><b><i>Powered by: geoffdevops.com</i></b></div>")
              .prepend(
        ...
        },
        {
          ...
          titleAttr: "Copy",
          title: window.store.name + " > Talla List",
          exportOptions: {
            columns: [0, 1, 2, 3],
          },
        },
    ...
    // Editar talla
    $(document).delegate("#edit-talla", "click", function (e) {
      e.stopPropagation();
      e.preventDefault();
    ...
    // Eliminar talla
    $(document).delegate("#delete-talla", "click", function (e) {
      e.stopPropagation();
      e.preventDefault();
      var datatable = dt;
    ...
    // Abrir el cuadro de diálogo modal de edición mediante cadena de consulta
    if (
      window.getParameterByName("talla_id") &&
      window.getParameterByName("talla_name")
    ) {
      supId = window.getParameterByName("talla_id");
      var supName = window.getParameterByName("talla_name");
      dt.DataTable().search(supName).draw();
      dt.DataTable().ajax.reload(function (json) {
        $.each(json.data, function (index, obj) {
          if (obj.DT_RowId === "row_" + supId) {
            TallaEditModal({ talla_id: supId, talla_name: obj.talla_name });
            return false;
          }
        });
      }, false);
    ...
    // Añadir botón de correo electrónico a los botones de la tabla de datos
    ...
    // Enviar lista de tallas por correo electrónico
