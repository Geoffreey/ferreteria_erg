...
// Guardar la factura en el log de auditor antes de eliminarla
...
// Obtener el n¨²mero de registros por p¨¢gina y el offset desde DataTables
$limit = isset($_GET['length']) && $_GET['length'] != '-1' ? intval($_GET['length']) : 1000000;  // Si es -1, se establece un l¨ªmite alto
$offset = isset($_GET['start']) ? intval($_GET['start']) : 0;

$table = "(SELECT * FROM selling_info WHERE {$where_query} ORDER BY created_at DESC LIMIT $limit OFFSET $offset) as selling_info";
